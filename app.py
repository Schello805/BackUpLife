...
def create_app() -> Flask:
    ...
    def before_request() -> None:
        ...
        allowed_endpoints = {"login", "setup", "static", "logo_asset", "profile_hint"}
        if (
...
def send_test_or_reset_mail(
    smtp_settings: sqlite3.Row, recipient: str, subject: str, body: str
) -> tuple[bool, str]:
...
def register_routes(app: Flask) -> None:
    ...
    def profile_hint(slug: str):
        ...
        if not profile:
            abort(404)
        return render_template("profile_hint.html", profile=profile)
    ...
    def inject_context() -> dict[str, Any]:
        active_profile = None
        visible_profiles = []
        if g.user and session.get("active_profile_slug"):
            active_profile = get_profile_by_slug(session["active_profile_slug"])
        if g.user:
